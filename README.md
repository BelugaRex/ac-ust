...
- **精确 home 隔离**：冷气状态读取、开机和页面定时器读写都只允许发生在精确 home URL；尾斜杠、查询串、哈希、相似路径和业务子页均拒绝作为初始操作目标。首次开机未确认时，统一恢复函数会刷新本次控制标签；若它已在等待期间偏离，则先回到精确 home，确认该精确页面完成加载后通过有限递归仅重试一次
