...
- **智能控制**：按香港天文台将军澳站（JKB）的气温、相对湿度、风速与站点雨量计算每个 30 分钟周期内的开启时长；露点由同站温湿度推导，雨量从无雨到黄雨阈值按后段更陡的指数曲线增强影响，最多把开启时间减半。灵敏度滑块提供 0–10 共 11 档（更省钱←→更干冷）；后台在 `:10`/`:50` 预取并生成分别绑定下一 `:30`/`:00` 的方案，方案保留原始天气，控制边界按当前灵敏度纯本地重算，不发起或等待天气网络。自动开启只在 `HH:00`/`HH:30` 的首分钟执行，错过则等待下一半点，单次最多开启 25 分钟并至少关闭 5 分钟。普通 PWM 循环仍可在任意分钟切换
...
- 天气改由严格未来的 `:10`/`:50` 一次性闹钟提前获取并预计算下一 `:30`/`:00`；方案保留原始天气，控制边界只消费目标时间完全匹配的本地方案，并按当前灵敏度纯本地重算，不直接或间接联网。预取失败会保留最近成功方案，但旧方案因目标不匹配不会被下一边界误用。
...
| 智能控制 | 在自动模式中选择后按天气计算开启时长；`:10`/`:50` 提前准备下一 `:30`/`:00` 的本地方案，边界不等待网络；自动 ON 仅在 `:00`/`:30` 首分钟执行且最多持续 25 分钟，错过边界则等待下一半点；OFF 仍由页面定时器按实际截止分钟执行 |
...
- 智能控制天气数据来自 **香港天文台开放数据 API**：将军澳站（Tseung Kwan O / JKB）的 1 分钟平均气温、1 分钟平均相对湿度、10 分钟平均风速，以及将军澳站过去 1 小时雨量；露点由同站气温与湿度用 Magnus 公式推导。后台使用严格未来的 one-shot 闹钟在 `:10`/`:50` 拉取并预计算下一 `:30`/`:00`，控制边界只消费目标完全匹配且足够新鲜的本地方案。预取失败不会覆盖最近成功方案，但目标不匹配的旧方案仍会被拒绝，本周期退回安全时长且不等待天气网络。
