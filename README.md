...
- **智能控制**：只按香港天文台将军澳站（JKB）的气温、相对湿度与风速计算每个 30 分钟周期内的开启时长；露点由同站温湿度推导，雨量不参与请求、缓存或控制决策。灵敏度滑块提供 0–10 共 11 档（更省钱←→更干冷）；后台在 `:10`/`:50` 预取并生成分别绑定下一 `:30`/`:00` 的方案，方案保留原始天气，控制边界按当前灵敏度纯本地重算，不发起或等待天气网络。自动开启只在 `HH:00`/`HH:30` 的首分钟执行，错过则等待下一半点，单次最多开启 25 分钟并至少关闭 5 分钟。普通 PWM 循环仍可在任意分钟切换
- **可用时刻预计**：仅在 Smart Power Meter 显示 `Charge Mode` 时读取冷气余额并计算分钟级预计时刻；其他计费模式不显示 est，避免误导。今明两日显示“今天／明天”，之后显示月日，预计 24 小时内用完时以轻量警示突出
...
- **清晰与低干扰**：固定使用与 UST 页面一致的浅色外观，提供清晰文字与语义状态反馈，并适配系统的减弱动态效果和高对比度
- **中英双语**：popup、后台与页面操作错误均支持中文/English，通过 Crowdin 社区翻译更多语言

## 0.8.3 更新要点

- 修复自动开启漏执行：自动 ON 先以固定绝对截止时间完成页面关机定时器预布防，成功后仅派发一次开启；单次输入失效不再终止整条开启链。
- 修复关机写入、重启与主世界递归 ON 的异步生命周期竞态，旧请求在停用/重启后不再继续落地。
- 智能天气输入仅使用将军澳站的气温、湿度与风速，雨量不再请求、缓存或参与决策。
- 新增打包构建限定的本机控制生命周期审计，逾期未确认的自动 ON 在诊断中明确提示。
...
- 智能天气输入收敛为将军澳 JKB 的独立实时气温、湿度与风速；不再请求、缓存或使用雨量，旧缓存中的雨量字段会被兼容忽略。
- 智能计算的 `K`、天气观测、`Teq` 与 `tRaw` 保留浮点精度，只在最终展示和控制值处量化分钟数。
...
| 智能控制 | 拨动开关按天气自动计算开启时长（与循环定时互斥，开启时自动折叠循环定时）；`:10`/`:50` 提前准备下一 `:30`/`:00` 的本地方案，边界不等待网络；自动 ON 仅在 `:00`/`:30` 首分钟执行且最多持续 25 分钟，错过边界则等待下一半点；OFF 仍由页面定时器按实际截止分钟执行 |
...
- 智能控制天气数据来自 **香港天文台开放数据 API**：只读取将军澳站（Tseung Kwan O / JKB）的 1 分钟平均气温、1 分钟平均相对湿度与 10 分钟平均风速；露点由同站气温与湿度用 Magnus 公式推导，雨量不参与请求、缓存或控制决策。后台使用严格未来的 one-shot 闹钟在 `:10`/`:50` 拉取并预计算下一 `:30`/`:00`，控制边界只消费目标完全匹配且足够新鲜的本地方案。预取失败不会覆盖最近成功方案，但目标不匹配的旧方案仍会被拒绝，本周期退回安全时长且不等待天气网络。
