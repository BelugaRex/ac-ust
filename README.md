...
- **智能控制**：按香港天文台天气（气温、湿度推导露点与雨量；风速缺失时按静风）自动计算每个 30 分钟周期（锚定半点）内的开启时长，用灵敏度滑块（0–10 共 11 档，更省钱←→更干冷）调节；开启时长限幅 0–30 分钟并做压缩机保护（1–4 分钟强制归零）
...
| 智能控制 | 拨动开关按天气自动计算开启时长（与循环定时互斥，开启时自动折叠循环定时）；灵敏度滑块实时预览建议分钟数（释放后生效，实际按 30 分钟周期执行） |
