...
- **智能控制**：按香港天文台天气（气温/湿度推导露点/风速/雨量）自动计算每个 30 分钟周期内的开启时长，用灵敏度滑块（更省钱←→更干冷）调节；开启时长限幅 0–30 分钟并做压缩机保护（1–4 分钟强制归零）
...
- 智能控制天气数据来自 **香港天文台开放数据 API**（`dataType=rhrread`）：气温优先取清水湾站（CWB，`rhrread` 暂无该实时站则回退西贡/将军澳站）、露点由气温 + 湿度用 Magnus 公式推导、雨量取西贡区过去 1 小时；风速开放数据不提供，按静风（0 m/s）保守处理。缓存 10 分钟并由后台每分钟续新。天气接口不可用时智能控制自动退回手动时长，不点击开关、不推进周期。
