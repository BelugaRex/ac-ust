...
- **智能控制**：按香港天文台将军澳站（JKB）的气温、相对湿度、风速与站点雨量计算每个 30 分钟周期内的开启时长；露点由同站温湿度推导，雨量从无雨到黄雨阈值按后段更陡的指数曲线增强影响，最多把开启时间减半。灵敏度滑块提供 0–10 共 11 档（更省钱←→更干冷）；自动开启只在 `HH:00`/`HH:30` 的首分钟执行，错过则等待下一半点，单次最多开启 25 分钟并至少关闭 5 分钟。普通 PWM 循环仍可在任意分钟切换
...
| 智能控制 | 拨动开关按天气自动计算开启时长（与循环定时互斥，开启时自动折叠循环定时）；自动 ON 仅在 `:00`/`:30` 首分钟执行且最多持续 25 分钟，错过边界则等待下一半点；OFF 仍由页面定时器按实际截止分钟执行 |
...
- 智能控制天气数据来自 **香港天文台开放数据 API**：将军澳站（Tseung Kwan O / JKB）的 1 分钟平均气温、1 分钟平均相对湿度、10 分钟平均风速，以及将军澳站过去 1 小时雨量；露点由同站气温与湿度用 Magnus 公式推导。缓存 1 小时并由后台整点续新；任一接口不可用时沿用最近缓存，没有缓存才自动退回手动时长，不点击开关、不推进周期。
