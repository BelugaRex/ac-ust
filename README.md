...
- **关机确认**：优先复用 AC home 页；只有全部标签偏离时才从 warning 等子页导航回入口。设置 `Power-off after` 后由独立新鲜页按 `10/15/20` 秒退避回读同一时间；未确认时自动重试
- **AC home 导航恢复**：开关指令会先确认标签位于 AC home；若标签停在 warning 等子页，或消息端口提前关闭，会导航原标签回 home、重新等待内容脚本就绪并仅重试一次；其他错误与页面未确认结果仍不重复发送
