...
- 智能控制天气数据来自 **香港天文台开放数据 API**（`dataType=rhrread`）：气温取将军澳站（Tseung Kwan O / JKB，回退西贡/清水湾/天文台）、露点由气温 + 湿度用 Magnus 公式推导、雨量取西贡区过去 1 小时；风速开放数据不提供，按静风（0 m/s）保守处理。缓存 10 分钟并由后台每分钟续新。天气接口不可用时智能控制自动退回手动时长，不点击开关、不推进周期。
