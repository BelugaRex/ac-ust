...
- **智能控制**：按香港天文台天气（气温、湿度推导露点与雨量；风速缺失时按静风）自动计算每个 30 分钟周期内的开启时长，用灵敏度滑块（0–10 共 11 档，更省钱←→更干冷）调节；自动开启只在 `HH:00`/`HH:30` 的首分钟执行，错过则等待下一半点，单次最多开启 25 分钟并至少关闭 5 分钟。普通 PWM 循环仍可在任意分钟切换
...
- 智能控制天气数据来自 **香港天文台开放数据 API**（`dataType=rhrread`）：气温取将军澳站（Tseung Kwan O / JKB，回退西贡/清水湾/天文台）、露点由气温 + 湿度用 Magnus 公式推导、雨量取西贡区过去 1 小时；风速开放数据不提供，按静风（0 m/s）保守处理。缓存 1 小时并由后台整点续新。天气接口不可用时智能控制自动退回手动时长，不点击开关、不推进周期。
