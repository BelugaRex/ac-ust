...
- **智能控制**：按香港天文台将军澳站（JKB）的气温、相对湿度、风速与站点雨量计算每个 30 分钟周期内的开启时长；露点由同站温湿度推导，雨量从无雨到黄雨阈值按后段更陡的指数曲线增强影响，最多把开启时间减半。灵敏度滑块提供 0–10 共 11 档（更省钱←→更干冷）；后台在 `:20`/`:50` 预取并生成分别绑定下一 `:30`/`:00` 的方案，方案保留原始天气，控制边界按当前灵敏度纯本地重算，不发起或等待天气网络。自动开启只在 `HH:00`/`HH:30` 的首分钟执行，错过则等待下一半点，单次最多开启 25 分钟并至少关闭 5 分钟。普通 PWM 循环仍可在任意分钟切换
...
- **自动确认弹窗**：浏览器原生对话框只在扩展发起的单次 AC 开机点击栈内自动处理，随后立即恢复；Ant Design 确认框、AC 开关与 `Power-off after` 控件必须能按页面语义唯一定位，歧义时拒绝猜测或点击
...
- **精确 home 隔离**：冷气状态读取、开机和页面定时器读写都只允许发生在精确 home URL；尾斜杠、查询串、哈希、相似路径和业务子页均拒绝作为初始操作目标。首次开机未确认时，统一恢复函数会刷新本次控制标签；若它已在等待期间偏离，则先回到精确 home，确认该精确页面完成加载后通过有限递归仅重试一次。智能 ON 首分钟已截止或自动控制已暂停时直接安全终止，不刷新／导航页面，也不误报为恢复异常
- **诊断面板**：一键查看闹钟、storage、content script 状态并自动修复常见问题；运行闹钟修复统一委派给带运行时段门禁的 Service Worker，popup 不直接创建闹钟。最近 50 条脱敏后台异常仅保存在本机，运行诊断后只会按新到旧附带当前构建以来最新 5 条
...
- 智能控制自动开启锚定 `:00`/`:30` 首分钟，页面关机截止固定为“半点 + 最多 25 分钟”；迟到不补开，已开启重试即使跨过下一半点也不会重新获得 25 分钟。普通循环定时不受半点门禁影响。
- 天气改由严格未来的 `:20`/`:50` 一次性闹钟提前获取并预计算下一 `:30`/`:00`；方案保留原始天气，控制边界只消费目标时间完全匹配的本地方案，并按当前灵敏度纯本地重算，不直接或间接联网。预取失败会保留最近成功方案，但旧方案因目标不匹配不会被下一边界误用。
...
| 操作 | 说明 |
|------|------|
| 自动模式 | 在「循环定时 / 智能控制」分段控件中二选一；再次点击已选模式会关闭自动控制 |
| 开启分钟 | 每次开启冷气持续多少分钟（修改后自动重启循环） |
| 关闭分钟 | 每次关闭冷气持续多少分钟（修改后自动重启循环） |
| 运行时段 | 独立作用于循环定时和智能控制；时段外保留所选模式并用页面定时器安全停机，重新进入后循环定时可立即恢复，智能控制等待下一次有效半点窗口 |
| 智能控制 | 在自动模式中选择后按天气计算开启时长；`:20`/`:50` 提前准备下一 `:30`/`:00` 的本地方案，边界不等待网络；自动 ON 仅在 `:00`/`:30` 首分钟执行且最多持续 25 分钟，错过边界则等待下一半点；OFF 仍由页面定时器按实际截止分钟执行 |
...
- 智能控制天气数据来自 **香港天文台开放数据 API**：将军澳站（Tseung Kwan O / JKB）的 1 分钟平均气温、1 分钟平均相对湿度、10 分钟平均风速，以及将军澳站过去 1 小时雨量；露点由同站气温与湿度用 Magnus 公式推导。后台使用严格未来的 one-shot 闹钟在 `:20`/`:50` 拉取并预计算下一 `:30`/`:00`，控制边界只消费目标完全匹配且足够新鲜的本地方案。预取失败不会覆盖最近成功方案，但目标不匹配的旧方案仍会被拒绝，本周期退回安全时长且不等待天气网络。
