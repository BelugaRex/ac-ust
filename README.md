...
- **智能模式**：按香港天文台天气（气温/湿度推导露点/风速/雨量）自动计算每个 30 分钟周期内的开启时长，用灵敏度滑块（弱冷←→强冷）调节，覆盖手动时长；开启时长限幅 0–30 分钟并做压缩机保护（1–4 分钟强制归零）
...
- **关机确认**：只复用 URL 完整等于 `https://w5.ab.ust.hk/njggt/app/home` 的标签；没有精确 home 时另建隐藏页，不改写 billing-cycle、warning、登录回调或其他 UST 页面。设置 `Power-off after` 后由独立新鲜页按 `10/15/20` 秒退避回读同一时间；未确认时自动重试
...
- **诊断面板**：一键查看闹钟、storage、content script 状态并自动修复常见问题；最近 50 条脱敏后台异常仅保存在本机，运行诊断后可连同最近 10 条异常一键复制
...
| 智能模式 | 拨动开关按天气自动计算开启时长；灵敏度滑块实时预览建议分钟数（释放后生效，实际按 30 分钟周期执行） |
...
├── smart-mode.js      # 智能模式纯决策（灵敏度→K、等效温度、开启分钟数，无 chrome.* 副作用）
├── offscreen.js       # 冗余保活心跳
...
- 智能模式天气数据来自 **香港天文台开放数据 API**（`dataType=rhrread`）：气温优先取清水湾站（CWB，`rhrread` 暂无该实时站则回退西贡/将军澳站）、露点由气温 + 湿度用 Magnus 公式推导、雨量取西贡区过去 1 小时；风速开放数据不提供，按静风（0 m/s）保守处理。缓存 10 分钟并由后台每分钟续新。天气接口不可用时智能模式自动退回手动时长，不点击开关、不推进周期。
...
- 诊断报告会附带本机最近 10 条后台异常；环形日志最多保留 50 条，只含时间、级别、代码阶段和截断后的脱敏错误消息，不记录页面 URL、标签 ID、DOM、余额或账号信息，也不会进入 `chrome.storage.sync` 或上传到服务器。
