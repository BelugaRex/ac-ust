...
- 用例 9–10：单一 ON 点击链路、所有读写只选择完整 URL 精确等于 AC home 的标签、其他 UST 页面不导航不发消息、消息端口提前关闭时改用隐藏 home 仅重试一次、OFF 零点击以及 ON→OFF 前的定时器证明
- 用例 11：`Power-off after` 写入后按 `10/15/20` 秒间隔在新鲜页面确认同一 `HH:MM`，避免过早加载验证页打断保存；失败重试、过期闹钟恢复、时钟修复与手动开机都不得绕过该确认
- 用例 15：Service Worker 后台异常按串行写链保存到本机环形日志，并发追加不丢失且只保留最新 50 条；URL/邮箱脱敏、消息截断、写入失败隔离，popup 诊断只并入最近 10 条
