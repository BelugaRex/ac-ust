...
- 用例 5–8：i18n 包体、冷气余额解析与 PWM 可用时间估算、跨设备相位同步（明确 OFF 安全投影、慢时钟停用、publish/adopt durable retry 与入出站交错最终收敛）、页面定时器解析与采纳、popup 布局防回归（CSS 禁 vw/vh，popup 固定为 280px 并保留固定 gutter，静态网页预览在窄窗口整体等比缩放）
...
- 用例 9–10：单一 ON 点击链路；本次点击前建立消息基线，并行等待确认框与新出现的精确 `Execution succeeded`，再结合 ON 状态确认完成。旧／隐藏／伪成功消息均不采信；已是 ON 显式返回 `alreadyDone`，零点击且直接进入页面 timer。原生对话框仅在单次点击栈内代理，AC 开关与 AntD 确认框按唯一语义定位；隐藏、无关、重复、禁用或歧义候选零点击。所有读写只选择完整 URL 精确等于 AC home 的标签；提示缺失、智能 ON 绝对截止或自动控制失效不通过刷新绕过，OFF 零点击
...
- 用例 16：运行时段作为循环定时与智能控制共用的独立门禁，覆盖 `[start,end)` 真值表、模式选择保留、循环立即恢复、可信半点 alarm 迟醒补执行／普通迟到 defer／页面 timer 安全余量、alarm 与 storage 原时刻转发、已 ON 零点击直达 timer、智能当前周期生命周期恢复与过迟反例、自动 ON/运行闹钟最终竞态复检、退出时页面定时器安全停机、独立关机 revision、主世界递归取消、停用状态下独立编辑无关机副作用，以及启动、同步、看门狗、诊断和 popup 暂停态旁路；另以确定性交错直接执行真实消息分支，验证本地设置／远端 sync 串行、显式慢时钟停用、publish/adopt 失败重试、重叠写最终 store 收敛、repair 同上下文合并与新 revision/boundary 尾随修复、智能灵敏度尾随、popup 最终提交失败／旧响应／旧轮询抑制、双模式 busy 反馈和时段外隐藏 Est.；popup 不得直接创建运行闹钟，自愈只委派后台
- 用例 17：五分钟舒适启动的纯目标计划、`false→true`／首次安装一次性触发、Popup／重启／更新反例、既有更晚 timer 读取与同页预置先于 ON、成功确认后的五分钟 floor 校准、新鲜证明与绝对 `ac-pwm` 对齐、已 ON 零点击、跨运行时段、失败重试、启动恢复和用户停用抢占；Popup storage 降级与诊断在舒适期内不误显示为暂停
- 用例 18：同一精确 tab 的“预置 timer→零刷新 ON→新鲜页证明”共享事务；直接执行生产函数轨迹，覆盖预置失败零点击、URL 漂移零点击、含糊但实际 ON 零追加点击、已 ON 零点击，以及 PWM／舒适／手动三入口复用
- 浏览器 E2E 从真实 Popup 在运行时段外启用自动控制，并另以真实扩展 Worker 验证先保险后开机：picker 歧义零点击；缺少新 toast 但页面稍后 ON 时只点击一次，且点击瞬间同一未刷新 tab 已持有目标 timer，随后由独立新鲜页确认。旅程还覆盖五分钟舒适期、停用一分钟保险、智能建议、四源天气、24h、中英文和完整重启
...
**用途**：用 Playwright 加载真实 `dist/` 扩展，覆盖 Popup 设置与校验、中文／英文布局和键盘、诊断及兜底复制、精确 AC home 状态／余额、先保险后 ON（含预置歧义与含糊实际 ON）、OFF 零点击、页面 timer 新鲜页证明、旧 receiver 恢复、完整浏览器重启、智能缓存消费，以及 HKO 四源请求→解析→缓存→边界计划→Popup 和单源失败回退。
