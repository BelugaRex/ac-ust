...
- 用例 9–10：单一 ON 点击链路；本次点击前建立消息基线，并行等待确认框与新出现的精确 `Execution succeeded`，再结合 ON 状态确认完成。旧／隐藏／伪成功消息均不采信；已是 ON 显式返回 `alreadyDone`，零点击且直接进入页面 timer。原生对话框仅在单次点击栈内代理，AC 开关与 AntD 确认框按唯一语义定位；隐藏、无关、重复、禁用或歧义候选零点击。所有读写只选择完整 URL 精确等于 AC home 的标签；提示缺失、智能 ON 绝对截止或自动控制失效不通过刷新绕过，真正的持续连接失败仍只有限恢复，OFF 零点击
...
- 用例 17：五分钟舒适启动的纯目标计划、`false→true`／首次安装一次性触发、Popup／重启／更新反例、ON 成功确认先于页面 timer、新鲜证明与绝对 `ac-pwm` 对齐、已 ON 零点击、既有更晚 timer 保留、跨运行时段、失败重试、启动恢复和用户停用抢占；Popup storage 降级与诊断在舒适期内不误显示为暂停
- 浏览器 E2E 从真实 Popup 在运行时段外启用自动控制：受控 OFF 页面只点击一次，短暂 `Execution succeeded` 后等待迟到 ON，再以独立新鲜页确认至少五分钟 timer；连续两次稳定快照后才继续快速模式／分钟操作。停用时再以第二张新鲜页确认一分钟关机保险，全旅程零 OFF 点击，并同时覆盖智能建议、四源天气、24h、中英文和完整重启
