...
- 用例 14：诊断将当前检查统一为 `ok/info/warning/repaired/error`，以稳定错误码、问题域、证据和唯一下一步生成摘要；自动控制关闭或运行时段暂停属于预期状态，不计入当前问题；后台自愈同时返回修复前快照和逐项修复记录；Popup 自检覆盖文档就绪/可见状态、视口与内容尺寸、横向溢出、控件与权威 `schedule` 同步以及 Popup→Service Worker 保活连接，正常纵向滚动不报错且不采集 URL、DOM 文本或账号信息
...
- 用例 9–10：单一 ON 点击链路、每次点击后等待 10 秒、原生对话框仅在单次点击栈内代理、AC 开关与 AntD 确认框按唯一语义定位；隐藏、无关、重复、禁用或歧义候选零点击，延迟出现的唯一 AC 确认框仍可轮询命中。所有读写只选择完整 URL 精确等于 AC home 的标签，其他 UST 页面不作为初始操作目标；首次开机未确认时由统一函数刷新或回到 home 并有限递归一次，智能 ON 截止或自动控制失效时零刷新／零恢复 ERROR，真正的持续连接失败仍只记录一次，OFF 零点击
