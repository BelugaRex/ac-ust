...
- 用例 9–10：单一 ON 点击链路、每次点击后等待 10 秒、原生对话框仅在单次点击栈内代理、AC 开关与 AntD 确认框按唯一语义定位；隐藏、无关、重复、禁用或歧义候选零点击，延迟出现的唯一 AC 确认框仍可轮询命中。所有读写只选择完整 URL 精确等于 AC home 的标签，其他 UST 页面不作为初始操作目标；首次开机未确认时由统一函数刷新或回到 home 并有限递归一次，OFF 零点击
