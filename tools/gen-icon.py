...
def render_toolbar_16():
    """Pixel-authored 16px mark with a 15px logical glyph."""
    navy = set()
    for y, x0, x1 in (
        (1, 4, 10), (2, 4, 11), (3, 3, 4), (4, 2, 4),
        (5, 2, 3), (5, 12, 13), (6, 1, 3), (6, 12, 13),
        (7, 1, 2), (7, 12, 14), (8, 1, 2), (8, 13, 14),
        (9, 1, 3), (9, 12, 14), (10, 2, 3), (10, 12, 13),
        (11, 2, 4), (11, 11, 13), (12, 3, 4), (12, 11, 12),
        (13, 4, 11), (14, 5, 10),
    ):
        navy.update((x, y) for x in range(x0, x1 + 1))
    # A 15px logical grid gives the snowflake one center pixel. The 16px
    # canvas therefore keeps padding on the top and right only.
    navy.add((0, 7))
    navy.update({(7, 15), (8, 15)})

    gold = {(11, 2)}
    gold.update((x, 3) for x in range(10, 13))
    gold.update((x, 4) for x in range(9, 14))
