...
修复智能自动开启偶发的「select-ok」失败：上次尝试遗留的时间选择下拉层未关闭，导致本次写入被误判为多个确认按钮而放弃；现在写入前会先关闭遗留下拉层。

### English

Fixes an occasional "select-ok" failure in smart auto-on where a leftover time-picker dropdown from a previous attempt was mistaken for multiple OK buttons; the dropdown is now closed before writing.
...
运行 `bash ./build.sh` 后，上传 `releases/ac-ust-v0.8.10.zip`。ZIP 内直接包含 `manifest.json`，没有额外的 `dist/` 外层目录。

## 发布流程

1. 运行构建与自动化测试，确认版本、ZIP、图标和本目录资料通过验证。
2. 上传 `releases/ac-ust-v0.8.10.zip`。
