...
• 精确 AC home 隔离：冷气状态读取、开机和页面定时器读写只允许发生在精确 home URL；尾斜杠、查询串、哈希、相似路径和业务子页均拒绝作为操作目标。消息端口提前关闭或现有 home 被浏览器丢弃时，扩展改用可回收的隐藏 home，仅重试一次且不导航用户标签
...
• Exact AC home isolation for status reads, startup, and page-timer access; trailing slashes, queries, hashes, lookalike paths, and business subpages are rejected as operation targets. If a message port closes early or an existing home tab is discarded, the extension retries once in a disposable hidden home tab without navigating user tabs
...
运行 `bash ./build.sh` 后，上传 `releases/ac-ust-v0.6.14.zip`。ZIP 内直接包含 `manifest.json`，没有额外的 `dist/` 外层目录。

## 发布流程

1. 运行构建与自动化测试，确认版本、ZIP 内容和图标均通过验证。
2. 在开发者信息中心上传 `releases/ac-ust-v0.6.14.zip`。
